...
abstract class Arr
{
	...
	public static function combine(
		array $keys, array $values, bool $overwrite = false
	) {
		if (count($keys) !== count($values)) {
			trigger_error('Both arrays must have same element count.');
			//
			return null;
		}
	...
	/**
	 * Extracts an array from the entries whose names matches the given prefix.
	 *
	 * @param iterable $source
	 * @param string $key
	 * @return array
	 */
	public static function extractPrefixedArray(iterable $source, string $key)
	{
		$result = [];

		$key = Str::endsWith($key, '.*') ? substr($key, 0, -2) : $key;

		$keylen = strlen($key);

		foreach ($source as $name => $value) {
			if (Str::startsWith($name, $key)) {
				$subname = substr($name, -(strlen($name) - $keylen - 1));

				if (array_key_exists($subname, $result)) {
					if (! is_array($result[$subname])) {
						$result[$subname] = array($result[$subname]);
					}
	...
	/**
	 * Extracts an array from the entries whose names matches the given suffix.
	 *
	 * @param iterable $source
	 * @param string $key
	 * @return array
	 */
	public static function extractSuffixedArray(iterable $source, string $key)
	{
		$result = [];

		$key = Str::startsWith($key, '*.') ? substr($key, 2) : $key;

		$keylen = strlen($key);

		foreach ($source as $name => $value) {
			if (Str::endsWith($name, $key)) {
				$subname = $name; //$key . '.' . substr($name, -(strlen($name) - $keylen + 1));
